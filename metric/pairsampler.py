...
class RandomNegative(_Sampler):
    def forward(self, embeddings, labels):
        with torch.no_grad():
            ...
            neg_idx = torch.multinomial(neg_mask.float()[anchor_idx], 1).squeeze(1)
...
class DistanceWeighted(_Sampler):
    ...
    """
    Distance weighted loss assume that embeddings are normalized by l2-norm.
    """
    ...
    def forward(self, embeddings, labels):
        with torch.no_grad():
            ...
            weight = weight * (neg_mask * (dist < self.nonzero_loss_cutoff).type(torch.uint8)).float()

            weight = weight + (((weight.sum(dim=1, keepdim=True) == 0).type(torch.uint8)) * neg_mask).float()
