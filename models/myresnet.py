...
class My_ResNet(nn.Module):
    def __init__(self, dataset, depth, num_branches, device, bottleneck=False, nl=False, embedding=False, se=False):
        ...
        self.gatelayer = GateBlock(fix_inplanes+(self.num_branches-1)*64*self.block.expansion, self.num_branches-1, bias=False)

        if self.nl:
            self.phi = nn.Conv2d(in_channels=32*self.block.expansion, out_channels=1, kernel_size=1)
            if self.embedding:
                self.theta = nn.Conv2d(in_channels=32*self.block.expansion, out_channels=32*self.block.expansion, kernel_size=1)

            for i in range(self.num_branches-1):
                setattr(self, 'phi_'+str(i), nn.Conv2d(in_channels=64*self.block.expansion, out_channels=1, kernel_size=1))
                if self.embedding:
                    setattr(self, 'theta_'+str(i), nn.Conv2d(in_channels=64*self.block.expansion, out_channels=64*self.block.expansion, kernel_size=1))
    ...
    def forward(self, x):
        ...
        for i in range(1, self.num_branches-1):
            ...
            ## Channel attention
            h_f = torch.cat([h_f, globals()['x_3_{}'.format(i)].unsqueeze(-1)], dim=-1)
            if self.nl:
        ...
        h_f = torch.cat([h_f, x_s.unsqueeze(-1)], dim=-1)
        ...
        return ind, x_en, x_c#, h_f

if __name__ == '__main__':
    model = My_ResNet('cifar100', depth=20, num_branches=4, nl=False, embedding=False, se=True)
    print(model)
    x = torch.randn(2, 3, 32, 32)
    y, x_m, x_c, h_f = model(x)
    print(y.size(), x_m.size(), x_c.size(), h_f.size())
