...
class My_DenseNet(nn.Module):
    def __init__(self, dataset, growth_rate=12, block_config=[16, 16, 16], num_branches=4,
        ...
        print(num_features)
        self.gatelayer = GateBlock(num_features+(self.num_branches-1)*132, self.num_branches-1, bias=True)
    ...
    def forward(self, x):
        ...
        context = self.avgpool(x_3_0).view(x_3_0.size(0), -1) # B x 132
        x_3_0 = getattr(self, 'classifier3_0')(context) # B x num_classes
        pro = x_3_0.unsqueeze(-1)
        for i in range(1, self.num_branches-1):
            ...
            context_temp = self.avgpool(temp).view(temp.size(0), -1)
            temp = getattr(self, 'classifier3_' + str(i))(context_temp)
            temp = temp.unsqueeze(-1)
            pro = torch.cat([pro, temp], -1)
            context = torch.cat([context, context_temp], dim=1)
        
        ## Peer attention
        # x_en = torch.mean(pro, dim=2)
        x_c = self.avgpool8x8(x)
        x_c = x_c.view(x_c.size(0), -1)
        x_c = torch.cat([x_c, context], dim=1)
        x_c = self.gatelayer(x_c)
        x_en = x_c[:,0].repeat(pro[:,:,0].size(1), 1).transpose(0, 1) * pro[:,:,0]
        for i in range(1, self.num_branches-1):
            x_en += x_c[:,i].repeat(pro[:,:,i].size(1), 1).transpose(0, 1) * pro[:,:,i]

        ## Student
        x_s = getattr(self, 'layer3_'+str(self.num_branches-1))(x) # B x 132 x 8 x 8
        x_s = self.avgpool8x8(x_s)
        x_s = x_s.view(x_s.size(0), -1)
        x_s = getattr(self, 'classifier3_'+str(self.num_branches-1))(x_s)
        x_s = x_s.unsqueeze(-1)
        pro = torch.cat([pro, x_s], -1) # B x categories x branches
