...
class CUB2011Classification(CUB2011):
    def __init__(self, root, train=False, transform=None, target_transform=None, download=False):
        ...
        if train:
            self.samples = [(img_file_pth, cls_ind) for img_file_pth, cls_ind in self.imgs if img_file_pth in train_list]
        else:
            self.samples = [(img_file_pth, cls_ind) for img_file_pth, cls_ind in self.imgs if img_file_pth in test_list]
...
class CUB2011Metric(CUB2011):
    ...
    def __init__(self, root, train=False, split='none', transform=None, target_transform=None, download=False):
        ...
        self.class_to_idx = {cls_name: cls_ind for cls_name, cls_ind in self.class_to_idx.items() if cls_name in self.classes}
        self.samples = [(img_file_pth, cls_ind) for img_file_pth, cls_ind in self.imgs if cls_ind in self.class_to_idx.values()]
        self.imgs = self.samples

if __name__=='__main__':
    cub200 = CUB2011Classification('/mnt/disk3/cub200/', train=True)
    print(len(cub200))
